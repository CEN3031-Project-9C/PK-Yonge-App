...
var path = require('path'),
  	...
  	User_SessionModel = mongoose.model('User_Session'),
...
exports.create = function (req, res) {
  var user_session = new User_SessionModel(req.body);
...
exports.list = function (req, res) {
  User_SessionModel.find().exec(function (err, user_sessions) {
    if (err) {
      return res.status(400).send({
        message: errorHandler.getErrorMessage(err)
      });
...
exports.userSessionById = function (req, res, next, id) {
  ...
  User_SessionModel.findById(id).exec(function (err, user_session) { //Populate finds objectId and replaces it with actual info.
    if (err) {
      return next(err);
